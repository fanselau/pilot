...
> **Repo:** `lucafanselau/pilot` at `~/dev/pilot/` (branch: `dev`)
...
|------|----------------|
| `~/dev/gsd-cli/.archive/gsd-bash` | 1219-line bash CLI — output formats for status, queue, stuck, log, tail |
| `~/dev/gsd-queue-v5.sh` | 667-line bash queue runner — lifecycle modes, state detection, process management |
| `~/dev/pilot/requirements/pilot-cli.md` | This file — the complete spec |
...
<project>/
├── .opencode/
│   ├── command/ → ~/dev/pilot-gsd/commands/
│   ├── agents/ → ~/dev/pilot-gsd/agents/
│   └── get-shit-done/ → ~/dev/pilot-gsd/get-shit-done/
├── opencode.json
└── .gitignore  (includes .opencode/)
```

**Error cases:**
- `$PILOT_GSD_DIR` doesn't exist → exit 1: `Error: pilot-gsd not found at <path>. Set PILOT_GSD_DIR or clone https://github.com/lucafanselau/pilot-gsd`
...
**Human output:**
```
✓ Created .opencode/command/ → ~/dev/pilot-gsd/commands/
✓ Created .opencode/agents/ → ~/dev/pilot-gsd/agents/
✓ Created .opencode/get-shit-done/ → ~/dev/pilot-gsd/get-shit-done/
✓ Created opencode.json
✓ Added .opencode/ to .gitignore
Setup complete: /home/user/dev/my-project
```
...
Pilot Configuration
────────────────────────────────────────────────────────
PILOT_QUEUE_FILE       ~/dev/QUEUE.md
PILOT_LOG_DIR          /tmp
PILOT_STUCK_THRESHOLD  90 (minutes)
PILOT_PROJECT_DIR      ~/dev
PILOT_GSD_DIR          ~/dev/pilot-gsd
NO_COLOR               (not set)
opencode               /home/user/.opencode/bin/opencode (found)
```
...
Reference: `~/dev/gsd-queue-v5.sh` functions `lifecycle_*`. Port these exactly.
