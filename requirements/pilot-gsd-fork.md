...
**Repo:** `punchlab-dev/pilot-gsd` at `~/dev/punchlab/pilot-gsd/`  
...
```bash
grep -rn '~/.claude/' ~/dev/punchlab/pilot-gsd/ --include='*.md' --include='*.json' | grep -v node_modules | grep -v '.git/'
```
Should return 0 results after patching.

```bash
grep -rn '/gsd:' ~/dev/punchlab/pilot-gsd/ --include='*.md' | grep -v node_modules | grep -v '.git/'
```
Should return 0 results after patching.
...
for f in ~/dev/punchlab/pilot-gsd/agents/*.md; do
...
cd /tmp/test-project
ln -s ~/dev/punchlab/pilot-gsd/agents .opencode/agents
ln -s ~/dev/punchlab/pilot-gsd/commands/gsd .opencode/command
ln -s ~/dev/punchlab/pilot-gsd/get-shit-done .opencode/get-shit-done
...
grep -rn 'AskUserQuestion' ~/dev/punchlab/pilot-gsd/commands/gsd/*.md
...
grep -B2 'AskUserQuestion' ~/dev/punchlab/pilot-gsd/get-shit-done/workflows/*.md | grep -v 'interactive\|Otherwise\|auto mode'
