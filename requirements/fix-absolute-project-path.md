...
## Problem
`pilot add` stores the full absolute path as `job.project` (e.g. `/home/luca/dev/punchlab/myapp`).
The runner at `src/core/runner.ts:118` does `path.join(config.projectDir, job.project)` which produces
a broken double-path like `/home/luca/dev/punchlab/home/luca/dev/punchlab/myapp`.
