...
// ── Exported keyboard handler for testability ────────────────────────────

function focusedPanelLength(state: import('./state.js').PilotStateStore): number {
...
function flashFor(state: import('./state.js').PilotStateStore, msg: string): void {
  state.setFlashMessage(msg);
  setTimeout(() => state.setFlashMessage(''), 2000);
}

export function handleKeyPress(
  key: TuiKeyEvent,
  state: import('./state.js').PilotStateStore,
  ...
  if (key.sequence === 'r') {
    ...
    if (job && (job.status === 'failed' || job.status === 'cancelled')) {
      ...
      flashFor(state, job ? `retry: not available (${job.status})` : 'retry: no job selected');
    }
    return;
  }
  ...
  if (key.sequence === 'x') {
    ...
    if (job && job.status === 'pending') {
      ...
      flashFor(state, job ? `cancel: not available (${job.status})` : 'cancel: no job selected');
    }
    return;
  }
  ...
  if (key.sequence === 'K') {
    ...
    if (job && job.status === 'running') {
      ...
      flashFor(state, job ? `kill: not available (${job.status})` : 'kill: no job selected');
    }
    return;
  }
  ...
  if (state.view() === 'dashboard') {
    const len = focusedPanelLength(state);
